...
export default {
    ...
    buildHead() {
        ...
        let headLinear = `<div class="months">${Object.keys(months).map(d => `<div style="width: ${139 * months[d]}px"><div class="month"><div class="label"><span>${['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'][parseInt(d.split('-')[0])]}</span><span class="year">${d.split('-')[1]}</span></div></div></div>`).join('')}</div>`;
    ...
    buildDay(day) {
        ...
        const html = `<div class="day${classname}" data-day="${day.getDay()}" data-date="${date}"><span>${day.getDate()} ${day.toLocaleDateString('en-US', { month: 'short' })}</span><div class="events"></div><div class="hours">${new Array(24).fill(0).map((_, i) => `<div class="hour" data-hour="${i.toString().padStart(2, '0')}:00"></div>`).join('')}</div></div>`;
